...
| 2026-04-24 | v0.6 Planning Baseline (Step 52-0 RFC) | `PreventionZoneTypeRegistry_v0.yaml` 中的 `spoil_disposal` (弃渣场区) 从 `extension_placeholders` 正式晋升至 `zone_types[]` tier=extended. 主导规范依据 GB 51018-2014 §5.7.1 + 表 5.7.1. 触发原因: `disposal_highrisk_v0` 样本在 Step 52-A PreventionSystem facts 回填时需使用该分区; 弃渣场与 `temp_soil_stockpile` (临时堆土区) 语义不可等价映射. **SoT 边界**: 弃渣场专业事实 (堆渣量 / 最大堆高 / 下游危害等级 / 评定级别) 仍归 `field.fact.disposal_site.*`, prevention.zones 内 `spoil_disposal` 仅作防治分区投影, 不复制专业事实. 本次 RFC 不动样本 / FIR / ArtifactRegistry / TABLE_PROJECTIONS. | Step 52-0 RFC |
